...
if (_unit isKindOf "Man") then {
		...
		if ((count _uniform) > 0) then {
			if (_uniformRandom == 1) then {
				...
				if(_uniformSel != uniform _unit) then {
					[_unit,_uniformSel] call pms_tb3_fnc_SetUniform;
				};
			} else {
				if(_uniform select 0 != uniform _unit) then {
					[_unit,_uniform select 0] call pms_tb3_fnc_SetUniform;
				};
			};
		} else {
			removeUniform _unit;
		...
		if ((count _backpack) > 0) then {
			if (_backpackRandom == 1) then {
				...
				if(_backpackSel != backpack _unit) then {
					[_unit,[_backpackSel]] call pms_tb3_fnc_Setbackpack;
				};
			} else {
				if(_backpack select 0 != backpack _unit) then {
					[_unit,[_backpack select 0]] call pms_tb3_fnc_Setbackpack;
				};
			};
		} else {
		...
		if ((count _vest) > 0) then {
			if (_vestRandom == 1) then {
				...
				if(_vestSel != vest _unit) then {
					[_unit,_vestSel] call pms_tb3_fnc_Setvest;
				};
			} else {
				if(_vest select 0 != vest _unit) then {
					[_unit,_vest select 0] call pms_tb3_fnc_Setvest;
				};
			};
		} else {
			removeVest _unit;
	...
	if ((count _headgear) > 0) then {
		if (_headgearRandom == 1) then {
			...
			[_unit,_headgearSel] call pms_tb3_fnc_SetHeadgear;
		} else {  [_unit,_headgear select 0] call pms_tb3_fnc_SetHeadgear; };
	};
	if ((count _goggles) > 0) then {
		if (_gogglesRandom == 1) then {
			_gogglesSel = _goggles call BIS_fnc_selectRandom;
			[_unit,_gogglesSel] call pms_tb3_fnc_SetGoggles;
		} else { [_unit,_goggles select 0] call pms_tb3_fnc_SetGoggles; };
	};

	if ((count _magazines) > 0) then {	[_unit,_magazines] call pms_tb3_fnc_SetMagazines; };
	if ((count _weapons) > 0) then { [_unit,_weapons,_priKit,_secKit,_pisKit] call pms_tb3_fnc_SetWeapons; };
	if ((count _items) > 0) then { [_unit,_items] call pms_tb3_fnc_SetItems;	};
  ...
  if (!(isNil "_aceEngineer")) then { _unit setVariable ["ACE_IsEngineer", _aceEngineer, true]; };
	...
	if (!(_unit isKindOf "Man")) then {
		...
		if ( local _unit ) then {
			...
			if ((count _vehCargoItems) > 0) then { [_unit,_vehCargoItems] call pms_tb3_fnc_setVehCargoItems; };
			if ((count _vehCargoWeapons) > 0) then { [_unit,_vehCargoWeapons] call pms_tb3_fnc_setVehCargoWeapons; };
			if ((count _vehCargoMagazines) > 0) then { [_unit,_vehCargoMagazines] call pms_tb3_fnc_setVehCargoMagazines; };
			if ((count _vehCargoRucks) > 0) then { [_unit,_vehCargoRucks] call pms_tb3_fnc_setVehCargoBackpacks; };
		};
	};
	...
	if ((isClass(configFile >> "CfgPatches" >> "ACRE_Main")) && (isClass(missionConfigFile >> "TB3_ACRE2")) && {(getNumber(TB3_Settings >> "ACRE2" >>  "babelEnabled") == 1)}) then {
		if ((count _languages) > 0) then { [_unit,_languages] call pms_tb3_fnc_setLanguages; };
	};
